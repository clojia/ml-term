...
class Preprocessor(object):
    ...
    def dataset_minmax(self, dataset):
        minVec = np.amin(dataset, axis=0)
        maxVec = np.amax(dataset, axis=0) 
        return minVec, maxVec
    
    def normalize_dataset(self, dataset, minVec, maxVec):
        for row in dataset:
            for i in range(len(row)):
                if maxVec[i].all() != minVec[i].all():
                    row[i] = (row[i] - minVec[i]) / (maxVec[i] - minVec[i])
        return dataset

    def getMatrix(self, data):
        matrix = self.convertDataMatrix(data)
       # print(matrix)
        minVec, maxVec = self.dataset_minmax(matrix)
    #    print(minVec)
     #   print(self.normalize_dataset(matrix, minVec, maxVec))
        return self.normalize_dataset(matrix, minVec, maxVec)
