...
class NeuralNetwork:
    ...
    """
    def __init__(self, numInput, numHidden, numOutput, seed, activation = 'comb'):
    ...
    def predict(self, xValues, validationOn=False):
        ...
        for j in range(self.numHidden):
            if self.activation == 'comb':
                if j == 0:
                    self.hiddenNodes[j] = NNUtil.hypertan(hSums[j])
                else:
                ...
                self.hiddenNodes[j] = NNUtil.leakyRelu(hSums[j])
        ...
        while epoch <= maxEpochs:
            ...
            for it in range(len(tData)):
               ...
               # print(pearsonSum)
                ...
                for j in range(self.numHidden):
                    sumW = 0.0 
                  #  print(str(other_sum) + " " + str(other_n) + " " + str(self.hiddenNodes[j-1]))
                    for k in range(self.numOutput):
                        sumW += outputErrorTerm[k] * self.hiddenOutputWeights[j, k] 
                    
                    if self.activation == 'comb':
                        if j == 0:
                            derivative = (1 - self.hiddenNodes[j]) * (1 + self.hiddenNodes[j])
                        else:
            ...
            if epoch == maxEpochs:
                ...
                for t, pair in enumerate(hiddenPairs):
                    ...
                    print("pearson correlation for pair " + str(t) + ": " + str(r))
            epoch += 1
            validationAcc = 0
            if vRatio < 1:
